...
RECORD_GENS = [1, 5, 10, 15, 20, 25, 29] 
FPS = 30 
...
# LOAD THEME (Need seed for map generation)
try:
...
def run_simulation(genomes, config):
    # This acts as the FITNESS FUNCTION for NEAT
    global GENERATION
    GENERATION += 1
    ...
    while running and len(cars) > 0:
        ...
        if should_record or frame_count % 10 == 0:
            ...
            screen.blit(font.render(f"{seconds}s / 58s", True, (255, 255, 255)), (20, 60))
...
def run_neat(config_path):
    ...
    # Save checkpoint every 5 generations so we don't lose progress
